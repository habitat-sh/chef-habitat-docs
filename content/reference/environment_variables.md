...
| `HAB_BLDR_CHANNEL` | build system, Supervisor | `stable` | Sets the Chef Habitat Builder channel to subscribe to. Defaults to `stable`. |
...
| `HAB_NOCOLORING` | build system | no default | If set to the lowercase string `"true"`, this environment variable unconditionally disables text coloring where possible. |
| `HAB_NONINTERACTIVE` | build system | no default | If set to the lowercase string `"true"`, this environment variable unconditionally disables interactive progress bars ("spinners") where possible. |
