...
| `HAB_CTL_SECRET` | Supervisor | no default | Shared secret used for [communicating with a Supervisor](../sup/sup_remote_control.md). |
...
| `HAB_SUP_UPDATE_MS` | Supervisor | 60000 | Interval in milliseconds governing how often to check for Supervisor updates when running with the [`--auto-update`](habitat_cli/#hab-sup-run) flag. Note: This variable will override the [`--auto-update-period`](habitat_cli/#hab-sup-run) flag. |
| `HAB_UPDATE_STRATEGY_PERIOD_MS` | Supervisor | 60000 | Interval in milliseconds governing how often to check for service updates when running with an [update strategy](../services/service_updates). Note: This variable will override the [--service-update-period](habitat_cli/#hab-sup-run) flag. |
