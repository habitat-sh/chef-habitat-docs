...
Configuration management tools (for example, [Ansible](https://www.ansible.com/), [Chef](https://www.chef.io/products/chef-infra), [Puppet](https://puppet.com/), and [Salt](https://saltproject.io/) let you write configuration files in a declarative language to manage a server.
...
As a result, Chef Habitat can replace many use cases that configuration management tools perform for installing system binaries, application-dependent libraries, or templating configuration files.

Configuration management tools perform tasks at runtime by converging resources.
The value of configuration management tools comes from this converging process---checking the existing state of a server, and fixing it if it doesn't match the intended state.
...
Chef Habitat avoids these classes of errors by shifting these processes to build time, and by creating an atomic package of an application's binaries, application lifecycle hooks, and configuration files.
...
Chef Habitat can make it easier to run your existing configuration management tools.
...
In this scenario, you can run the [Chef Habitat Supervisor](sup) on your existing virtual machine or bare metal infrastructure as you migrate away from your configuration management tools.
Then, when you're ready, you can export your application to the container format of your choice using the [Chef Habitat Studio](../packages/pkg_build.md).
While you migrate your applications and services, the [Chef Habitat Supervisor](sup) runs on your existing mutable infrastructure and runs your existing configuration management tools.
