...
`pkg_source`
: A URL that specifies the location from which to download an external source. Any valid `wget` URL will work. Typically, the relative path for the URL contains the `pkg_name` and `pkg_version` values. Type: URL. _Optional_.
