...
[menu.builder]
    title = "Origin keys"
    identifier = "builder/on-prem/origins/keys"
    parent = "builder/on-prem/origins"
    weight = 30
+++
...
- [Download and installed the Chef Habitat CLI](/install/).
- [Create a Chef Habitat On-prem Builder account](/builder/saas/builder_account/).
- [Generate a personal access token](/builder/saas/builder_profile/#create-a-personal-access-token).
...
For more information, see the [`hab origin key` CLI documentation](/reference/habitat_cli/#hab-origin-key).
...
- Use [`hab setup`](/install/hab_setup/) to generate your first origin key pair as part of setting up the `hab` CLI.
- Use the `hab origin key generate` command to create an key pair for an origin created with the `hab origin create` command

To create an origin key with the `hab` CLI, run the [`hab origin key generate` command](/reference/habitat_cli/#hab-origin-key-generate):
...
To download your public origin key using the command line, use [`hab origin key download` command](/reference/habitat_cli/#hab-origin-key-download):
...
Upload origin keys with the [`hab origin key upload` command](/reference/habitat_cli/#hab-origin-key-upload):
...
Use [`hab origin key import`](/reference/habitat_cli/#hab-origin-key-import) to read the key from a standard input stream into Chef Habitat On-prem Builder:
