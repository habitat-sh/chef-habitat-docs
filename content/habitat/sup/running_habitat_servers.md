...
First, you must [install Chef Habitat](/habitat/install/) itself on the machine.
...
As with Linux, you must first [install Chef Habitat](/habitat/install/) on the machine. Unlike Linux, however, the Windows Supervisor has no requirements for any `hab` user.
